...
export default function Programmers() {
  ...
  const [ allProgrammers ] = useState(listOfAwesome)
  ...
  const getNameOfFeatured = () => {
   ...
   return allProgrammers.find(elem => elem.id === featured).name;
  };
      ...
      <div className='programmers'>
        {
          ...
          allProgrammers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => 
                { /* in here set the featured id to be dev.id */ setFeatured(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
